  ...
  return (
    <View style={styles.contain}>
      <View style={[styles.content, { backgroundColor: colors.card }]}>
        <Text title3 medium style={{ padding: 10 }}>Add Your Ads</Text>
        {/* <View style={styles.wrapContent}>
          {business.map((item, index) => {
            const color = selected == item.key ? colors.primary : colors.card;
            return (
              <TouchableOpacity onPress={() => setSelected(item.key)} style={{ alignItems: "center", padding: 10 }} key={item.key}>
                <Image style={[styles.image, { borderColor: color }]} source={item.image} />
                <Text footnote style={{ marginTop: 10, }}>{item.title}</Text>
              </TouchableOpacity>
            );
          })}
        </View> */}
        ...
        <View style={[styles.contentButton, { borderTopColor: colors.border }]}>
          <TouchableOpacity style={styles.button} onPress={() => navigation.goBack()}>
            <Text body2 semibold>{t("close")}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, { borderLeftColor: colors.border, borderLeftWidth: 0.5, },]} onPress={onSaveBusiness}
          >
            <Text body2 semibold>{t("done")}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
